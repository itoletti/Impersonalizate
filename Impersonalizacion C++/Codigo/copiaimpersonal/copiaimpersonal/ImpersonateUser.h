...
//namespace darka
//{
	__declspec (dllexport) class ImpersonateUser
	{
	private:
		bool init_;
		HANDLE userToken_;
	...
	public:
		__declspec (dllexport) ImpersonateUser() : init_(false), userToken_(NULL), errorCode_(0) {}
		__declspec (dllexport) ~ImpersonateUser();

		__declspec (dllexport) bool Logon(const std::string& userName, const std::string& domain, const std::string& password);
		__declspec (dllexport) void Logoff();

		// Misc Methods
		__declspec (dllexport) DWORD GetErrorCode() const { return errorCode_; }
	};
//} // namespace
